...
static int xilinx_vdma_probe(struct platform_device *pdev)
{
	...
	// save the start and end address on the config structure    
	XAxiVdma_Config_ptr->mem_phy_start = r_mem->start ;
	XAxiVdma_Config_ptr->mem_phy_end  = r_mem->end  ;

	dev_set_drvdata(dev, XAxiVdma_Config_ptr);  
	...
	subdev=device_create(cls, NULL, MKDEV(major, 0), NULL, "vdma"); 
	if (IS_ERR(subdev))
	{
	...
	/*            
	 ...
	 NOTE :   here is how I store the 480 image 
